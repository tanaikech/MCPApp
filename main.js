...
/**
 * Global instance for managing the MCPApp class.
 * @private
 */
var mcpAppInstance = null;
...
function mcpApp(object) {
  mcpAppInstance = new MCPApp(object);
  return mcpAppInstance;
}
...
function setServices(services) {
  if (!mcpAppInstance) {
    throw new Error("[MCP Error] Please initialize MCPApp by calling mcpApp(object) first.");
  }
  return mcpAppInstance.setServices(services);
}
...
function server(object) {
  ...
  return mcpAppInstance.server(object);
}
...
function client(object) {
  if (!mcpAppInstance) {
    mcpAppInstance = new MCPApp();
  }
  mcpAppInstance.clientObject = object;
  return mcpAppInstance.client(object);
}
...
function callMCPServers() {
  if (!mcpAppInstance) {
    throw new Error("[MCP Error] Please initialize MCPApp by calling mcpApp(object) and client(object) first.");
  }
  return mcpAppInstance.callMCPServers();
}
